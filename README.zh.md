...
`bot.type_text` 等）同样注入真实输入，所以也会点亮光晕——第一次调用时亮起，
最后一次调用后几秒淡出，连续的脚本化操作显示为一段完整的接管，而不是闪烁。
...
错误；任务被记录为**已取消（cancelled）**而非失败，主动中止不会污染你的
失败率统计。中止具**粘性**：同一 bot 实例后续所有 `bot.ai()` 都会立即抛错
（不点光晕、不碰键鼠），脚本里的 try/except 无法把你刚收回的机器再抢回去
——要继续必须显式调用 `bot.clear_user_abort()`；单步调用不受影响，可用于
善后清理。短促的 ESC 点击——无论是你按的还是 bot 注入的——都不会触发。这个
