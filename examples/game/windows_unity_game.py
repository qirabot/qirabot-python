...
Pattern:
  ...
  3. Bind to that HWND via Airtest's ``Windows:///<hwnd>`` so input and
     screenshots target *this* window even when other apps overlay it.
...
def find_game_hwnd(wnd_class: str) -> int | None:
    ...
    def cb(hwnd: int, _: object) -> None:
        if win32gui.IsWindowVisible(hwnd) and win32gui.GetClassName(hwnd) == wnd_class:
            found.append(hwnd)
...
def main() -> None:
    ...
    print(f"connecting to hwnd={hwnd}")
    device = connect_device(f"Windows:///{hwnd}")
